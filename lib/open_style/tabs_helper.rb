  ...
  module TabsHelper
    ...
    module InstanceMethods
      ...
      def current_tab(namespace = nil)
        tab_stack[namespace.to_sym || :tabs]
      end
    ...
    module HelperMethods
      ...
      # Behind the scenes, each time you create a tab, the <tt>#item</tt> 
      # method is invoked.
      #
      #   <% tabs_tag do |tab| %>
      ...
      #
      # See <tt>OpenStyle::TabsBuilder#item</tt> for more details.
      #
      ...
      def tabs_tag(options = {}, &block)
        TabsBuilder.new(self, { :namespace => :tabs }.merge(options)).render(&block)
      end

      def navigation_tag(options = {}, &block)
        TabsBuilder.new(self, options.merge({ :namespace => :navigation })).render(&block)
      end

      def menu_tag(options = {}, &block)
        TabsBuilder.new(self, options.merge({ :namespace => :menu })).render(&block)
      end

      def submenu_tag(options = {}, &block)
        TabsBuilder.new(self, options.merge({ :namespace => :submenu })).render(&block)
      end
    end
  end
end
